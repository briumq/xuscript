...
- lexer：
  ...
  - 把规范 28 关键字全部纳入关键字 token（含 `let/var/does/pub/static/self/in/use/as/has/with` 等），避免靠 Ident 文本特判。
- parser：
...
## 风险点与控制
- 关键字收敛会影响"把 let/var/pub 等当作标识符"的旧代码；这是符合规范的破坏性变更，会通过全量 tests/specs 兜底。
