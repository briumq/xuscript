...
| 属性/方法 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `.length` | 获取字符串长度（属性） | 无 | 整数 | ✅ 已实现 |
| `length()` | 获取字符串长度（方法） | 无 | 整数 | ✅ 已实现 |
...
| 属性/方法 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `.length` | 获取列表长度（属性） | 无 | 整数 | ✅ 已实现 |
| `length()` | 获取列表长度（方法） | 无 | 整数 | ✅ 已实现 |
| `first()` | 获取第一个元素 | 无 | Option[元素] | ✅ 已实现 |
| `push()` | 向列表追加元素 | 元素 | unit | ✅ 已实现 |
| `pop()` | 移除并返回最后一个元素 | 无 | 被移除的元素 | ✅ 已实现 |
| `reverse()` | 反转列表 | 无 | unit | ✅ 已实现 |
| `join()` | 用指定分隔符连接列表元素 | 分隔符 | 连接后的字符串 | ✅ 已实现 |
| `contains()` | 检查列表是否包含指定元素 | 元素 | 布尔值 | ✅ 已实现 |
| `clear()` | 清空列表 | 无 | unit | ✅ 已实现 |
| `remove()` | 按索引删除元素并返回该元素 | 索引 | 被移除的元素 | ✅ 已实现 |
| `insert()` | 在指定位置插入元素 | 索引, 元素 | unit | ✅ 已实现 |
| `sort()` | 对列表排序 | 无 | unit | ✅ 已实现 |
| `get()` | 安全获取指定索引的元素 | 索引 | Option[元素] | ✅ 已实现 |
| `map()` | 对列表元素应用函数 | 函数 | 新列表 | ✅ 已实现 |
| `filter()` | 过滤列表元素 | 谓词函数 | 新列表 | ✅ 已实现 |
| `reduce()` | 归约列表元素 | 函数, 初始值 | 归约结果 | ✅ 已实现 |
| `find()` | 查找第一个匹配元素 | 谓词函数 | Option[元素] | ✅ 已实现 |
| `find_index()` | 查找第一个匹配元素的索引 | 谓词函数 | Option[int] | ✅ 已实现 |
| `find_or()` | 查找第一个匹配元素，未找到返回默认值 | 谓词函数, 默认值 | 元素 | ✅ 已实现 |

### 2. 字典 (dict)

| 属性/方法 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `.length` | 获取字典长度（属性） | 无 | 整数 | ✅ 已实现 |
| `length()` | 获取字典长度（方法） | 无 | 整数 | ✅ 已实现 |
| `merge()` | 合并另一个字典 | 字典 | 合并后的字典 | ✅ 已实现 |
| `insert()` | 插入键值对 | 键, 值 | unit | ✅ 已实现 |
...
| `clear()` | 清空字典 | 无 | unit | ✅ 已实现 |
| `contains()` | 检查是否包含指定键 | 键 | 布尔值 | ✅ 已实现 |

### 3. 元组 (tuple)

| 属性/方法 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `.length` | 获取元组长度（属性） | 无 | 整数 | ✅ 已实现 |
| `length()` | 获取元组长度（方法） | 无 | 整数 | ✅ 已实现 |
| `.0`, `.1`, `.2` ... | 按索引访问元素（属性） | 无 | 元素 | ✅ 已实现 |
| `[0]`, `[1]`, `[2]` ... | 按索引访问元素（下标） | 无 | 元素 | ✅ 已实现 |

| 特性 | 描述 | 实现状态 |
|------|------|----------|
| 元组创建 | 使用括号创建元组，如 `(1, "hello", 3.14)` | ✅ 已实现 |
| 解包操作 | 支持元组解包到多个变量，如 `let (a, b) = (1, 2)` | ✅ 已实现 |
| 迭代支持 | 支持在 for 循环中迭代元组元素 | ✅ 已实现 |
| 不可变性 | 元组内容不可修改 | ✅ 已实现 |
...
Option 类型用于表示可能存在或不存在的值，有两个变体：`Option#some(value)` 和 `Option#none`。

| 属性/方法 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `.has` | 检查是否有值（属性） | 无 | 布尔值 | ✅ 已实现 |
| `has()` | 检查是否有值（方法） | 无 | 布尔值 | ✅ 已实现 |
| `.name` | 获取变体名称（属性） | 无 | 字符串 | ✅ 已实现 |
| `.type_name` | 获取类型名称（属性） | 无 | 字符串 | ✅ 已实现 |
| `get()` | 获取内部值，None 时报错 | 无 | 值 | ✅ 已实现 |
| `or()` | 如果是 None，则返回指定值 | 默认值 | 值 | ✅ 已实现 |
| `or_else()` | 如果是 None，则执行指定函数并返回其结果 | 函数 | 值 | ✅ 已实现 |
| `map()` | 对内部值应用函数并返回新的 Option | 函数 | Option[函数返回值] | ✅ 已实现 |
| `then()` | 对内部值应用函数并返回其结果（flatMap） | 函数 | 函数返回值 | ✅ 已实现 |
| `each()` | 对内部值执行副作用函数 | 函数 | unit | ✅ 已实现 |
| `filter()` | 根据条件过滤内部值 | 谓词函数 | Option[值] | ✅ 已实现 |
| `name()` | 获取变体名称（方法） | 无 | 字符串 | ✅ 已实现 |
| `type_name()` | 获取类型名称（方法） | 无 | 字符串 | ✅ 已实现 |
| `to_string()` | 转换为字符串表示 | 无 | 字符串 | ✅ 已实现 |

**示例：**
```xu
let opt = Option#some(42)
println(opt.has)        // true
println(opt.get())      // 42
println(opt.name())     // "some"
println(opt.type_name()) // "Option"
println(opt.to_string()) // "Option#some(42)"

let none_opt = Option#none
println(none_opt.has)   // false
println(none_opt.or(0)) // 0
```
...
println(s.name)        // "pending"
println(s.type_name)   // "Status"
println(s.name())      // "pending"
println(s.type_name()) // "Status"
println(s.to_string()) // "Status#pending"
```
...
2. 属性访问语法：`object.property`
3. `length` 同时支持属性访问（`.length`）和方法调用（`length()`）
4. Option 的 `has` 同时支持属性访问（`.has`）和方法调用（`has()`）
5. 枚举和结构体类型支持使用 `does` 关键字绑定自定义方法
6. 方法调用时，方法名会被转换为对应的 `MethodKind` 枚举值，然后根据接收对象的类型分发到具体的实现函数
...
```xu
let numbers = [1, 2, 3]
numbers.push(4)          // 现在 numbers = [1, 2, 3, 4]
println(numbers.pop())   // 输出: 4, 现在 numbers = [1, 2, 3]
println(numbers.length)  // 输出: 3 (属性访问)
println(numbers.length()) // 输出: 3 (方法调用)
println(numbers.first()) // 输出: Option#some(1)
```
...
println(status.name)        // 输出: approved
println(status.type_name)   // 输出: Status
println(status.to_string()) // 输出: Status#approved
```
...
| 类型 | 属性 | 方法 |
|------|------|------|
| **List** | `.length` | `length()`, `first()`, `push()`, `pop()`, `get()`, ... |
| **Dict** | `.length` | `length()`, `keys()`, `values()`, `items()`, `get()`, ... |
| **String** | `.length` | `length()`, `split()`, `trim()`, ... |
| **Tuple** | `.length`, `.0`, `.1`, ... | `length()` |
| **Option** | `.has`, `.name`, `.type_name` | `has()`, `get()`, `or()`, `map()`, ... |
| **Enum** | `.name`, `.type_name` | `name()`, `type_name()`, `to_string()` |

### 设计原则

1. **属性与方法并存**：`length` 和 `has` 同时支持属性访问和方法调用，方便不同编程习惯的用户
2. **语义清晰**：方法名采用完整的英文单词，如 `starts_with()` 而非 `starts()`
3. **跨语言一致性**：命名风格参考 JavaScript 和 Ruby 的习惯
4. **面向对象风格**：优先使用方法形式而非函数形式