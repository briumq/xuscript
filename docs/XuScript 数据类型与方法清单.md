...
| 其他类型 | 部分支持 | 元组、集合、符号、范围、正则表达式等 | 缺少多种高级数据类型 |
...
| `starts_with()` / `starts()` | 检查字符串是否以指定前缀开头 | 前缀 | 布尔值 | ✅ 已实现 |
| `ends_with()` / `ends()` | 检查字符串是否以指定后缀结尾 | 后缀 | 布尔值 | ✅ 已实现 |
| `len()` / `length()` | 获取字符串长度 | 无 | 整数 | ✅ 已实现 |
| `find()` | 查找子串位置 | 子串 | Option[int] | ❌ 未实现 |
| `substr()` | 提取子串 | 起始位置, 长度 | 字符串 | ❌ 未实现 |
| `match()` | 正则表达式匹配 | 正则表达式 | Option[列表] | ❌ 未实现 |
| `replace_all()` | 替换所有匹配子串 | 旧子串, 新子串 | 字符串 | ❌ 未实现 |
| `trim_start()` | 去除字符串开头空白 | 无 | 字符串 | ❌ 未实现 |
| `trim_end()` | 去除字符串结尾空白 | 无 | 字符串 | ❌ 未实现 |
...
| 方法名 | 描述 | 参数 | 返回值 | 实现状态 |
|--------|------|------|--------|----------|
| `add()` / `push()` | 向列表追加元素 | 元素 | 修改后的列表 | ✅ 已实现 |
| `pop()` | 移除并返回最后一个元素 | 无 | 被移除的元素 | ✅ 已实现 |
| `reverse()` | 反转列表 | 无 | 反转后的列表 | ✅ 已实现 |
| `join()` | 用指定分隔符连接列表元素 | 分隔符 | 连接后的字符串 | ✅ 已实现 |
| `len()` / `length()` | 获取列表长度 | 无 | 整数 | ✅ 已实现 |
| `contains()` | 检查列表是否包含指定元素 | 元素 | 布尔值 | ✅ 已实现 |
| `clear()` | 清空列表 | 无 | 空列表 | ✅ 已实现 |
| `remove()` | 按索引删除元素并返回该元素 | 索引 | 被移除的元素 | ✅ 已实现 |
| `insert()` | 在指定位置插入元素 | 索引, 元素 | 修改后的列表 | ❌ 未实现 |
| `sort()` | 对列表排序 | 无 | 排序后的列表 | ❌ 未实现 |
| `map()` | 对列表元素应用函数 | 函数 | 新列表 | ❌ 未实现 |
| `filter()` | 过滤列表元素 | 谓词函数 | 新列表 | ❌ 未实现 |
| `reduce()` | 归约列表元素 | 函数, 初始值 | 归约结果 | ❌ 未实现 |
| `find()` | 查找第一个匹配元素 | 谓词函数 | Option[元素] | ❌ 未实现 |
...
| `remove()` | 删除指定键的键值对 | 键 | Option[值] | ❌ 未实现 |
| `get_or_default()` | 获取指定键的值，若不存在则返回默认值 | 键, 默认值 | 值 | ❌ 未实现 |
| `items()` | 获取所有键值对 | 无 | 键值对列表 | ❌ 未实现 |
| `clear()` | 清空字典 | 无 | 空字典 | ❌ 未实现 |
...
### 1. 数学运算与类型转换
- 整数和浮点数缺少数学运算方法（如 `abs()`, `round()`, `floor()`, `ceil()` 等）
- 缺少进制转换方法

### 2. 字符串处理
- 缺少正则表达式支持
- 缺少更高级的字符串操作（如 `find()`, `substr()`, `replace_all()` 等）
