...
| `filter` | `filter(|T| -> bool) -> [T]` | 返回满足条件的新列表 |
| `map` | `map(|T| -> U) -> [U]` | 返回映射后的新列表 |
...
> **注意**：以下标准库模块（math/time/json/file/sys）为**实验性功能**，API 可能在后续版本中发生变化。这些模块作为实现参考提供，尚未纳入正式语言规范。
