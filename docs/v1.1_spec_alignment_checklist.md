...
| `list.push(x)` | `list.add(x)` 或 `list.push(x)` | 多个文件 (push 现已作为 add 的别名) |
...
| `when ... { } ` (无 else) | `when ... { } else { }` | language_features_demo.xu |
...
| Result.map 返回结构体失败 | `result.map(func(x) -> Struct { Struct{...} })` | `RuntimeError: Unsupported member access: field on type void` | ✅ 已修复 | 现在可以正常使用 |
...
```xu
// ❌ 会报错
return parse_role(type_str).map(func(role: Role) -> Token {
