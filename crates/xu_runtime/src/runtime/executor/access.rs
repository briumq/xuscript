use crate::Value;
use crate::value::{DictKey, i64_to_text_fast};
...
impl Runtime {
    ...
    pub(crate) fn get_member_with_ic_raw(
        ...
        if tag == crate::value::TAG_STRUCT {
        ...
        } else if tag == crate::value::TAG_LIST && field == "first" {
            let id = obj.as_obj_id();
        ...
        } else if tag == crate::value::TAG_TUPLE && (field == "len" || field == "length") {
            let id = obj.as_obj_id();
            if let crate::gc::ManagedObject::Tuple(v) = self.heap.get(id) {
                Ok(Value::from_i64(v.len() as i64))
            } else {
            ...
            let id = obj.as_obj_id();
            let keys_raw: Vec<crate::Text> = if let crate::gc::ManagedObject::Dict(db) =
                self.heap.get(id)
            {
            ...
            for s in keys_raw {
                keys.push(Value::str(self.heap.alloc(crate::gc::ManagedObject::Str(s))));
            }
            Ok(Value::list(self.heap.alloc(crate::gc::ManagedObject::List(keys))))
            ...
            Ok(Value::list(self.heap.alloc(crate::gc::ManagedObject::List(values))))
            ...
            for (k, v) in raw_items {
                let key = Value::str(self.heap.alloc(crate::gc::ManagedObject::Str(k)));
                let pair = Value::tuple(
                    self.heap
                        .alloc(crate::gc::ManagedObject::Tuple(vec![key, v])),
                );
                items.push(pair);
            }
            Ok(Value::list(self.heap.alloc(crate::gc::ManagedObject::List(items))))
    ...
    pub(crate) fn get_index_with_ic(
        ...
        if tag == crate::value::TAG_DICT {
            let id = obj.as_obj_id();
            let (cur_ver, key_hash, key) = if let crate::gc::ManagedObject::Dict(me) = self.heap.get(id)
            {
    ...
    pub(crate) fn get_index_with_ic_raw(
        ...
        if tag == crate::value::TAG_LIST {
            ...
            if let crate::gc::ManagedObject::Dict(me) = self.heap.get(id) {
                if index.get_tag() == crate::value::TAG_STR {
                    let key = if let crate::gc::ManagedObject::Str(s) = self.heap.get(index.as_obj_id())
                    {
                        s.as_str().to_string()
                    } else {
                        return Err(self.error(xu_syntax::DiagnosticKind::GetKeyRequired));
            ...
            if index.is_int() {
                ...
                let id = obj.as_obj_id();
                let s = if let crate::gc::ManagedObject::Str(s) = self.heap.get(id) {
                    s.as_str().to_string()
                } else {
                ...
                let (start, end, inclusive) = if let crate::gc::ManagedObject::Range(a, b, inc) =
                    self.heap.get(id)
                {
                    (*a, *b, *inc)
                } else {
                ...
                let s = if let crate::gc::ManagedObject::Str(s) = self.heap.get(sid) {
                    s.as_str().to_string()
                } else {
                    return Err(self.error(xu_syntax::DiagnosticKind::Raw("Not a string".into())));
                };
