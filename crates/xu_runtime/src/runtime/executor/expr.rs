...
use crate::Text;
use crate::Value;
use crate::value::{Dict, DictKey, Function, StructInstance, UserFunction, i64_to_text_fast};

use super::closure_analysis::needs_env_frame;
use super::super::appendable::Appendable;
use super::super::util::to_i64;
    ...
    pub fn eval_expr(&mut self, expr: &Expr) -> Result<Value, String> {
        match expr {
            ...
            Expr::List(items) => {
                ...
                Ok(Value::list(self.heap.alloc(crate::gc::ManagedObject::List(v))))
            }
            Expr::Tuple(items) => {
                if items.is_empty() {
            ...
            Expr::Range(r) => {
                ...
                Ok(Value::range(
                    self.heap
                        .alloc(crate::gc::ManagedObject::Range(start, end, r.inclusive)),
                ))
            }
            Expr::IfExpr(e) => {
            ...
            Expr::Match(m) => {
                ...
                for (pat, body) in m.arms.iter() {
                    if let Some(binds) = super::super::pattern::match_pattern(self, pat, &v) {
                        if self.locals.is_active() {
                            self.push_locals();
                ...
                for item in s.items.iter() {
                    match item {
                        ...
                        xu_ir::StructInitItem::Spread(e) => {
                            ...
                            if base.get_tag() == crate::value::TAG_STRUCT {
